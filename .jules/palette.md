...
## 2025-06-24 - [A11y: Accesibilidad de contadores y aria-describedby]
**Learning:** El uso de `aria-hidden="true"` en elementos que sirven como referencia para `aria-describedby` anula la capacidad de los lectores de pantalla para anunciar el contenido asociado. Para que un contador de caracteres sea útil y accesible, no debe estar oculto para el árbol de accesibilidad si está vinculado a un input.
**Action:** Asegurar que los elementos de feedback (como contadores) no tengan `aria-hidden="true"` si están asociados semánticamente a otros controles mediante atributos ARIA.
