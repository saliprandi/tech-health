...
**Aprendizaje:** El badge de 'Estado de Horario Comercial' es un patrón micro-UX highly efectivo para sitios basados en servicios. Proporciona certeza inmediata al usuario sobre la disponibilidad del laboratorio sin requerir que interpreten un horario. Usar un script del lado del cliente con offsets UTC asegura precisión a través de diferentes zonas horarias de usuarios manteniendo el contexto local de Tucumán.
...
**Aprendizaje:** Agregar un contador de caracteres en tiempo real a textareas con límites strictly de longitud (como 500 caracteres) proporciona feedback crítico que previene la frustración del usuario durante el envío de formularios. Usar `aria-describedby` vincula el contador al input para lectores de pantalla, mientras que evitar `aria-live="polite"` en el contador mismo previene verbosidad excesiva durante cada pulsación de tecla.
...
## 2026-06-27 - [UX: Prevención de colisión de texto y contadores en inputs]
**Learning:** En elementos de entrada con contadores de caracteres flotantes absolutos (`absolute bottom-* right-*`), la falta de padding interno adecuado hace que el texto ingresado por el usuario se superponga con el contador. Agregar un padding inferior y derecho suficiente (ej., `pb-7 pr-14`) al `<textarea>` y aplicar `pointer-events-none select-none` al contador asegura legibilidad y permite que los clics en esa área pasen al campo de texto.
**Action:** Al agregar contadores de caracteres flotantes dentro del contenedor de un campo de texto, aplicar siempre padding amplio en las esquinas correspondientes y `pointer-events-none` al contador para evitar obstrucciones visuales e interactivas.
