...
## 2025-05-18 - [Payload Reduction via Global Deduplication]
**Learning:** Massive code duplication and script corruption in key components (Nav, Footer, Contacto, Servicios) not only blocked the build but also bloated the HTML payload by over 18KB (~25%). Deduplicating these components and consolidating scripts is a critical performance win in this repository.
**Action:** Always audit the build output size after fixing component corruption to quantify the payload savings.
