...
## 2026-06-19 - Evitar micro-optimizaciones en rutas frías de interacción
**Aprendizaje:** Intentar optimizar handlers de eventos de clic aislados (como bucles cortos $O(N)$ de 5 elementos o creación dinámica de elementos `<a>` en eventos de usuario) constituye una micro-optimización en una ruta fría ("cold path") sin impacto medible. Además, reemplazar elementos `<a>` por `window.open` con parámetros adicionales puede alterar el comportamiento del navegador (abriendo popups en lugar de pestañas) e introducir regresiones.
**Acción:** Enfocar siempre las optimizaciones en rutas calientes de renderizado, scroll o timers. Si la aplicación ya está altamente optimizada y no hay cuellos de botella claros, no realizar cambios que degraden la mantenibilidad o el comportamiento predeterminado del navegador.
