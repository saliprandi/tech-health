...
## 2026-03-30 - Control de estado y navegación en botones de envío de formularios nativos
**Learning:** Establecer de forma sincrónica `submitBtn.disabled = true` dentro de un manejador de evento `submit` en formularios con navegación nativa (ej. `GET /estado`) provoca la cancelación de la navegación en ciertos motores de navegador (WebKit/Chrome) al deshabilitar el elemento emisor antes de que el evento sea despachado. Usar una bandera de control de estado (`isSubmitting`) evita re-envíos duplicados por teclado o ratón, mientras que `aria-busy="true"`, `aria-disabled="true"` y la clase `pointer-events-none` ofrecen retroalimentación visual y accesible sin interrumpir la navegación nativa.
**Action:** Evitar `disabled = true` sincrónico en eventos de envío con navegación nativa y combinar flags JS de sumisión con atributos `aria-busy`, `aria-disabled` y clases de desactivación de eventos de puntero.
