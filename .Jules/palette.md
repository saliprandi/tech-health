...
## 2026-03-30 - Preservación de navegación nativa en formularios GET y gestión de carga accesibles
**Learning:** Deshabilitar síncronamente el botón de envío (`submitBtn.disabled = true`) dentro del manejador del evento `submit` en formularios con navegación nativa HTML (ej. GET `/estado?ticket=...`) puede cancelar el envío nativo del navegador en motores como WebKit o Firefox. Asimismo, escuchar el evento nativo `invalid` en el `<input>` permite activar `aria-invalid="true"` de forma automática sin romper la validación estándar del navegador.
**Action:** En formularios de navegación nativa GET, usar una bandera de estado (`isSubmitting`) junto con `aria-disabled="true"`, `aria-busy="true"` e ícono spinner en lugar de `disabled=true` síncrono, y sincronizar la propiedad `aria-invalid="true"` con el evento `invalid` del input.
