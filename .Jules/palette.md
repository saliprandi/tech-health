...
## 2025-05-22 - Global Skip to Content Link Implementation
**Learning:** Adding a "Skip to Content" link at the layout level ensures consistent accessibility for keyboard users across all pages, but it requires a universal target ID (e.g., `<main id="main-content">`) within the same layout to prevent broken links on subpages that use the same layout.
**Action:** When implementing skip-to-content links in shared layouts, wrap the `<slot />` or main content area with the target container and ID in the layout file itself.
