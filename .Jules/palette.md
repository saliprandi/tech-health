...
## 2026-03-30 - Selección de texto en recuperación de errores y descarte con Escape en formularios de consulta
**Learning:** Al desplegar un mensaje de error en una búsqueda fallida de ticket (`src/pages/estado.astro`), redirigir el foco de vuelta al campo de entrada y seleccionar su texto (`input.focus(); input.select();`) permite al usuario sobrescribir de inmediato la entrada inválida sin necesidad de borrar carácter por carácter. Asimismo, habilitar la tecla `Escape` en el campo para limpiar el valor y descartar alertas proporciona una atajo accesible y estándar.
**Action:** En campos de consulta o búsqueda, auto-seleccionar el texto inválido al capturar errores y soportar la tecla `Escape` para restablecer el campo y descartar estados de error.
