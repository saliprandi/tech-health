...
## 2026-03-30 - Inhabilitación accesible en envíos nativos de formulario sin cancelar navegación
**Learning:** En manejadores de eventos de envío de formularios nativos HTML (`<form method="GET">`), inhabilitar síncronamente el botón mediante `submitBtn.disabled = true` durante el evento `submit` aborta la navegación de la página en navegadores estrictos. Utilizar un flag de estado (`isSubmitting`) combinado con `aria-disabled="true"`, `aria-busy="true"`, íconos de carga animados (`animate-spin`) y la clase `pointer-events-none` previene envíos duplicados y otorga retroalimentación accesible inmediata a lectores de pantalla sin interrumpir la navegación nativa GET.
**Action:** Utilizar `aria-disabled="true"`, `aria-busy="true"` y banderas de control de envío en lugar de `button.disabled = true` en manejadores de formularios HTML que realizan navegación GET.
